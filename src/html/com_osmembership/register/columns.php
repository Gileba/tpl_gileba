...
if (OSMembershipHelper::isJoomla4() && $this->config->twitter_bootstrap_version != "uikit3") {
...
?>
<div id="osm-singup-page-columns" class="osm-container<?php echo $containerClass; ?> osm-plan-<?php echo $this->plan
 	->id; ?> clearfix">
	<?php
 ...
 ?>
    <div id="osm-signup-page-columns-form">
        <?php // Hidden login form using to pass login information from subscription form to to process login

if (!$this->userId && $this->config->show_login_box_on_subscribe_page) {
        	echo $this->loadTemplate("login_container");
        } ?>
        <form method="post" name="os_form" id="os_form" action="<?php echo Route::_(
        	"index.php?option=com_osmembership&task=register.process_subscription&Itemid=" . $this->Itemid,
        	false,
        	$this->config->use_https ? 1 : 0
        ); ?>" enctype="multipart/form-data" autocomplete="off" class="<?php echo $formClass .
	$additionalFormClass; ?>">
            <div class="<?php echo $bootstrapHelper->getClassMapping("row-fluid"); ?>">
                <div class="col-md-7">
                    ...
                    <div id="osm-columns-payment-information">
		                <?php
                  if (
                  	(isset($this->fees["amount"]) && $this->fees["amount"] > 0) ||
                  	$this->form->containFeeFields() ||
                  	$this->plan->recurring_subscription
                  ) { ?>
                   		...
                   		echo $field->getControlGroup($bootstrapHelper, true);
                            ...
                            <div class="<?php echo $controlGroupClass; ?> osm-coupon-container">
                                ...
                                <div class="<?php echo $controlsClass; ?>">
                                    <input type="text" class="form-control validate[required] <?php echo $bootstrapHelper->getClassMapping(
                                    	"input-medium"
                                    ); ?>" name="coupon_code" id="coupon_code" value="<?php echo $this->escape(
	$this->input->getString("coupon_code", "")
); ?>" />
                                </div>
                            </div>
			            <?php }
                  ...
                  if (
                  	$this->config->show_privacy_policy_checkbox ||
                  	$this->config->show_subscribe_newsletter_checkbox
                  ) {
                  ...
                  ?>
                        <div class="form-actions">
                            <input type="submit" class="<?php echo $btnPrimaryClass; ?>" name="btnSubmit" id="btn-submit" value="<?php echo Text::_(
	$buttonText
); ?>">
                            <img id="ajax-loading-animation" src="<?php echo Uri::root(
                            	true
                            ); ?>/media/com_osmembership/ajax-loadding-animation.gif" style="display: none;" alt="<?php echo Text::_(
	"OSM_ALT_LOADING"
); ?>" />
                        </div>
                    ...
                    <div class="col-md-5">
                                                        <div id="osm-columns-subscription-information">
                                                        <?php // Login form for existing user

if (!$this->userId && $this->config->show_login_box_on_subscribe_page) {
                                                        	echo $this->loadCommonLayout(
                                                        		"register/tmpl/columns_login.php",
                                                        		["fields" => $fields]
                                                        	);
                                                        } ?>
                                                        </div>
            ...
            <input type="hidden" name="default_country" id="default_country" value="<?php echo $this->config
            	->default_country; ?>" />
            <input type="hidden" id="card-nonce" name="nonce" />
	        <?php echo $this->loadCommonLayout("register/tmpl/default_anti_spam.php"); ?>
            <?php echo HTMLHelper::_("form.token"); ?>

	        <?php if ($this->checkoutTrackingCode) {
